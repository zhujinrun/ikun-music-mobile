...
const SourceDetail = ({
  ...
  onConfirm: (info: LX.Music.MusicInfoOnline) => void
  toggleSource: LX.Music.MusicInfoOnline | null
}) => {
  const theme = useTheme()
...
const Modal = forwardRef<ModalType, {}>((props, ref) => {
  ...
  const loadData = useCallback(
    (selectInfo: SelectInfo = infoRef.current) => {
  ...
  const confirmToggleSource = useCallback(async (musicInfo: LX.Music.MusicInfoOnline) => {
    const isClose = await handleToggleSource(
      infoRef.current.listId,
      ...
      <View style={styles.container}>
        {sourceInfo.sourceInfo.length ? (
          <>
            ...
            <List source={source} lists={sourceInfo.lists} onPlay={handlePlay} />
          </>
        ) : (
          <Empty loading={sourceInfo.loading} error={sourceInfo.error} onReload={loadData} />
        )}
        <SourceDetail
          info={infoRef.current.musicInfo}
          onConfirm={confirmToggleSource}
